...
<script>
				let cnt = 1;
				function fn_addFile() {
					$("#d_file").append("<input type='file' id='iq_image"+cnt+"' name='iq_image"+cnt+"' required='required'>");
					
					
					$("#iq_image"+cnt+"").change(function () {
...
<body>
	...
	<div class="container col-lg-4">
        ...
        <div class="inputArea contentBox">
			...
			<div id="d_file">
				<input type="file" id="basic_iq_image" name="basic_iq_image" required="required">
			<script>
			$("#basic_iq_image").change(function () {
                if (this.files && this.files[0]) {
                    let reader = new FileReader;
                    reader.onload = function (data) {
                    	$('.select_img').append("<img src='' class='basic_iq_img' />")
                        $(".select_img .basic_iq_img").attr("src", data.target.result).width(100);
                    }
                    reader.readAsDataURL(this.files[0]);
                }
            });
